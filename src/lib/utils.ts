...
export function mapCallableError(err: unknown): string {
  if (err && typeof err === "object") {
    ...
    if (code.includes("failed-precondition") && message.toLowerCase().includes("livekit")) {
      return "Voice server is not configured. An admin must deploy LiveKit credentials to Firebase (npm run deploy:voice).";
    }
    ...
    if (code.includes("internal")) {
      return message && message !== "internal"
        ? message
        : "Voice server error. LiveKit may not be configured on Firebase — run deploy:voice with functions/.env.";
    }
