...
import type { ChessLeaderboardEntry } from "@/models";

export default function LeaderboardPage() {
  const [data, setData] = useState<{ topTopics: { name: string; score: number }[]; topChats: { name: string; score: number }[] } | null>(null);
      ...
      <section>
        ...
        <div className="space-y-2">
          {data?.topTopics.map((t, i) => (
            <div key={t.name} className="flex justify-between rounded-lg border border-white/10 p-3">
      ...
      <section>
        ...
        <div className="space-y-2">
          {data?.topChats.map((c, i) => (
            <div key={c.name} className="flex justify-between rounded-lg border border-white/10 p-3">
